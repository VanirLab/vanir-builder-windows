# Vanir builder - preparing Windows build environment for Vanir
#
# If launched outside of existing vanir-builder, clones it to current directory.
# Existing vanir-builder location may be specified via `-builder <path>' option.
# Build environment is contained in 'msys' directory created in vanir-builder/cache/windows-prereqs. It also contains mingw64.
...
if ($builder)
{
    # use pased value for already existing vanir-builder directory
    $builderDir = $builder

    $logFilePath = Join-Path (Join-Path $builderDir "build-logs") "win-initialize-be.log"
    Start-Transcript -Path $logFilePath

    Write-Host "[*] Using '$builderDir' as vanir-builder directory."
}
else # check if we're invoked from existing vanir-builder
{
    ...
    if (($curDir -eq "scripts") -and (Test-Path -Path $makefilePath))
    {
        ...
        Write-Host "[*] Running from existing vanir-builder ($builderDir)."
    }
    else
    {
        Start-Transcript -Path "win-initialize-be.log"
        Write-Host "[*] Running from clean state, need to clone vanir-builder."
    }
}
...
if (! $builder)
{
    # fetch vanir-builder off the repo
    $repo = "git://github.com/$GIT_SUBDIR/vanir-builder.git"
    $builderDir = Join-Path $scriptDir "vanir-builder"
    Write-Host "[*] Cloning vanir-builder to $builderDir"
...
# move msys/mingw to vanir-builder/cache/windows-prereqs, this will be the default "clean" environment
...
if ($verify)
{
	...
	Write-Host "[*] Importing Vanir OS signing keys..."
	# import master vanir signing key
	& $gpg --keyserver hkp://keys.gnupg.net --recv-keys 0x36879494

	# import other dev keys
	$file = Join-Path $builderDir "vanir-developers-keys.asc"
	& $gpg --import $file

	# add gpg and msys to PATH
	$env:Path = "$env:Path;$msysDir\bin;$gpgDir\pub"

	# verify vanir-builder tags
	$tag = & git tag --points-at=HEAD | head -n 1
	$ret = & git tag -v $tag
	if ($?)
	{
		Write-Host "[*] vanir-builder successfully verified."
	}
	else
	{
		Write-Host "[!] Failed to verify vanir-builder! Output:`n$ret"
		Exit 1
	}
}
# set msys to start in vanir-builder directory
...
CreateShortcuts "vanir-msys.lnk" "$msysDir\msys.bat"
...
echo $wdkPath
& $makecertPath -sv $builderDir\qwt.pvk -n "CN=Vanir Test Cert" $builderDir\qwt.cer -r
